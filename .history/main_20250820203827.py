...
if __name__ == '__main__':
    ...
    for run_id, seed, split_index in zip(*run_loop_settings()):
        ...
        auto_select_device()
        # Enforce GPU usage: fail fast if not running on CUDA
        try:
            use_cuda = torch.cuda.is_available()
        except Exception:
            use_cuda = False
        if not use_cuda or ('cuda' not in str(getattr(cfg, 'device', 'cpu')).lower()):
            raise RuntimeError(
                f"GPU is required but not available. cfg.device={getattr(cfg, 'device', 'unknown')}, "
                f"torch.cuda.is_available()={use_cuda}. Ensure CUDA_VISIBLE_DEVICES is set and CUDA is installed.")
